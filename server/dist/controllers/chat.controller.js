...
const ChangeGroupChatTagline = AsyncHandler(async (req, res, next) => {
...
const AddNewMembersToGroupChat = AsyncHandler(async (req, res, next) => {
    ...
    const cacheKeysToDelete = [
        `get-group-chats-${user?.userId}`,
        ...memberIds.map((id) => `get-group-chats-${id}`),
        ...newMemberIds.map((id) => `get-group-chats-${id}`),
    ];
    await Promise.all(cacheKeysToDelete.map((key) => cache.delCache(key)));
...
const AddNewMemberToGroupChat = AsyncHandler(async (req, res, next) => {
...
const RemoveMemberFromGroupChat = AsyncHandler(async (req, res, next) => {
...
const MakeMemberAdmin = AsyncHandler(async (req, res, next) => {
    const { chatId, memberId } = req.body;
    const user = req.user;
    if (!chatId || !memberId) {
        return res.status(400).json(new ResponseHandler(400, "Invalid input data.", {}));
...
const DismissAdmin = AsyncHandler(async (req, res, next) => {
    const { chatId, memberId } = req.body;
    const user = req.user;
    if (!chatId || !memberId) {
        return res.status(400).json(new ResponseHandler(400, "Invalid input data.", {}));
...
const GetConnectionChats = AsyncHandler(async (req, res, next) => {
    const user = req.user;
    if (!user?.userId)
    ...
    if (await cache.hasCache(cacheKey)) {
        ...
        connections = chats.flatMap((chat) => chat.users
    ...
    const unreadMessages = await prisma.message.groupBy({
        by: ['chatId'],
        where: {
            chatId: { in: connections.map((c) => c.chatId) },
...
const GetGroupChats = AsyncHandler(async (req, res, next) => {
    const user = req.user;
    if (!user?.userId)
        return next(new ErrorHandler("User not found.", HTTP_STATUS_NOT_FOUND));
...
const GetInfiniteScrollMessages = AsyncHandler(async (req, res, next) => {
