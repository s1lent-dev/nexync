...
const sendConnectionRequest = AsyncHandler(async (req, res, next) => {
    ...
    if (userId === user?.userId) {
        return next(new ErrorHandler("You cannot send connection request to yourself", HTTP_STATUS_BAD_REQUEST));
    }
    ...
    if (requestExists) {
        return next(new ErrorHandler("Request already exists", HTTP_STATUS_BAD_REQUEST));
    }
    ...
    res
        .status(HTTP_STATUS_OK)
        .json(new ResponseHandler(HTTP_STATUS_OK, "Request sent successfully", {}));
});
const unsendConnectionRequest = AsyncHandler(async (req, res, next) => {
    const { userId } = req.params;
    const user = req.user;
    ...
    res
        .status(HTTP_STATUS_OK)
        .json(new ResponseHandler(HTTP_STATUS_OK, "Request unsend successfully", {}));
});
const acceptConnectionRequest = AsyncHandler(async (req, res, next) => {
    ...
    if (status === "rejected") {
        ...
        return res
            .status(HTTP_STATUS_OK)
            .json(new ResponseHandler(HTTP_STATUS_OK, "Request rejected successfully", {}));
    }
    await prisma.connectionRequest.delete({
        where: { requestId: request.requestId },
    });
    ...
    if (chatExists) {
        return res
            .status(HTTP_STATUS_OK)
            .json(new ResponseHandler(HTTP_STATUS_OK, "Request accepted successfully", {}));
    }
    ...
    await prisma.userChat.createMany({
        data: [
            { userId: user?.userId, chatId: chat.chatId },
...
const removeFollowers = AsyncHandler(async (req, res, next) => {
    const { userId } = req.params;
    const user = req.user;
    const chatExists = await prisma.chat.findFirst({
        where: {
...
const getMyConnections = AsyncHandler(async (req, res, next) => {
    const user = req.user;
    if (!user?.userId) {
    ...
    if (await cache.hasCache(cacheKey)) {
        ...
        followingData = following.map((f) => {
            const isFollower = followers.some((follower) => follower.followerId === f.following.userId);
            return {
        ...
        followersData = followers.map((f) => {
            const isFollowing = following.some((followed) => followed.followingId === f.follower.userId);
            const isRequested = connectionRequests.some((req) => req.receiverId === f.follower.userId && req.status === "pending");
            return {
    ...
    res.status(200).json(new ResponseHandler(200, "Connections fetched successfully", {
        followingData,
        followersData,
    }));
});
const getAllConnections = AsyncHandler(async (req, res, next) => {
    ...
    return res
        .status(HTTP_STATUS_OK)
        .json(new ResponseHandler(HTTP_STATUS_OK, "Connections found.", connections));
});
...
const getConnectionRequests = AsyncHandler(async (req, res, next) => {
    const user = req.user;
...
const updateUsername = AsyncHandler(async (req, res, next) => {
    ...
    if (!result) {
        return next(new ErrorHandler("Unable to update username", HTTP_STATUS_BAD_REQUEST));
    }
    const { canUpdate, remainingDays } = result;
    if (!canUpdate) {
        return next(new ErrorHandler(`You can update your username after ${remainingDays} days`, HTTP_STATUS_BAD_REQUEST));
    }
    ...
    const deleteCachePromises = prefixes.map((prefix) => cache.delCachePattern(`${prefix}:*`));
    await Promise.all(deleteCachePromises);
    res.json(new ResponseHandler(HTTP_STATUS_OK, "Username updated successfully", {}));
});
const uploadAvatar = AsyncHandler(async (req, res, next) => {
    ...
    if (!filePath || !contentType || !fileName) {
        return next(new ErrorHandler("File not uploaded", HTTP_STATUS_BAD_REQUEST));
    }
    ...
    res.json(new ResponseHandler(HTTP_STATUS_OK, "File uploaded successfully", avatarUrl));
});
...
export { getMe, searchUsers, sendConnectionRequest, unsendConnectionRequest, acceptConnectionRequest, removeFollowers, removeFollowing, getMyConnections, getAllConnections, getSuggestions, getConnectionRequests, updateUsername, uploadAvatar, updateBio, };
