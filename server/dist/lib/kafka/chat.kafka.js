...
class ChatKafkaService extends KafkaService {
    constructor() {
        super();
        this.consumer1 = this.createConsumer(KAFKA_GROUP_ID1);
        this.consumer2 = this.createConsumer(KAFKA_GROUP_ID2);
        this.topic1 = KAFKA_TOPIC1;
        this.topic2 = KAFKA_TOPIC2;
    }
        ...
        await this.consumer1.run({
            eachMessage: async ({ topic, partition, message, pause }) => {
                try {
                    ...
                    const msg = await JSON.parse(messageValue);
    ...
    async publishMessageRead(message) {
        await this.producer.send({
            topic: this.topic2,
            messages: [{ key: `msg-${Date.now()}`, value: JSON.stringify(message) }],
        });
    ...
    async consumeMessageRead() {
        ...
        await this.consumer2.run({
            eachMessage: async ({ topic, partition, message, pause }) => {
                try {
                    const messageValue = message.value ? message.value.toString() : "null";
                    const msg = JSON.parse(messageValue);
